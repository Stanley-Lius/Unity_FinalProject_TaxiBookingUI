...
A real-time **multiplayer taxi management simulation** built with **Unity** and **Photon Fusion**, featuring spline-based road networks, Dijkstra pathfinding, AI traffic, and a server-authoritative networking architecture.

https://github.com/user-attachments/assets/REPLACE_WITH_YOUR_DEMO_VIDEO_ASSET_ID

> Upload `Demo.mp4` to your GitHub repo, then replace the link above with the actual video URL.
...
|---|---|
| **Engine** | Unity 6 (URP) |
| **Networking** | Photon Fusion v2 — Client-Host architecture |
| **Pathfinding** | Custom Dijkstra's algorithm on spline graph |
| **AI Traffic** | Autonomous vehicles with collision avoidance |
| **Road System** | Unity Splines — 48+ curves for natural road geometry |
| **UI** | World-space billboard displays (speed, ETA) |
...
- Custom **graph builder** that converts spline knots into a weighted, bidirectional graph
...
- AI taxis (`CarPlayer`) randomly select destinations and navigate independently
- **Raycast-based collision avoidance** with adaptive speed control — smooth deceleration when obstacles are detected ahead
